...
var LdapHandler = function(options)
{
    ...
    this.handleGroups = function(req, res)
    {
        if('one' == req.scope) {
            ...
            sendFilteredObjects(req, res, [lof.getGroupsBase()]);
        } else {
            sendFilteredObjects(req, res, [lof.getGroupsBase()]);
            sendFilteredObjects(req, res, lof.getGroups());
        }
    ...
    this.handleUsers = function(req, res) {
        if('one' == req.scope) { // List Call for Accounts
            ...
            sendFilteredObjects(req, res, [lof.getUsersSBASE()]);
        } else {
            sendFilteredObjects(req, res, [lof.getUsersBase()]);
            sendFilteredObjects(req, res, lof.getUsers());
       }
    };
    ...
    this.handleBase = function(req, res) {
        if('base' == req.scope) {
            ...
            sendFilteredObjects(req, res, [lof.getGroupsBase(), lof.getUsersBase()]);
        }
        else if('sub' == req.scope) {
            sendFilteredObjects(req, res, [lof.getGroupsBase(), lof.getUsersBase(), lof.getRootObj()]);
            sendFilteredObjects(req, res, lof.getGroups());
            sendFilteredObjects(req, res, lof.getUsers());
        }
    };
